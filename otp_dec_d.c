...
void handleIncoming(int connection){
    ...
    //identify yourself!!
    write(connection, "dec", 3);

    char *ciphertext, *keytext, *deciphered;
    //char buffer[1024];
    int n;

    //first call to getFile stores ciphertext in a temp file
    ...
    //call decode and pass it the new tempfiles and the connection
    deciphered = decode(connection, ciphertext, keytext);

    send_file(deciphered, connection);
...
char* decode(int connection, char *cipherfile, char *keyfile){
    ...
    //set up one last file to hold the deciphered text
    FILE *temp;
    int size = 24;
    char *tempname = malloc(size);
    int pid = getpid();
    snprintf(tempname, size, "%s%d", "deciphered", pid);
